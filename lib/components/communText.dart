...
class CommunFun {
  ...
  static getShiftId() async {
    var isjoin = await CommunFun.checkIsJoinServer();
    var shiftid;
    if (isjoin == false) {
      shiftid = await Preferences.getStringValuesSF(Constant.DASH_SHIFT);
    } else {
      shiftid = await Preferences.getStringValuesSF(Constant.SERVER_SHIFT_ID);
    }
    return shiftid;
  }

  static setserverSettings() async {
    var branchid = await branchapi.getBranchid();
    var shiftid = await shiftAPI.getshiftID();
    await Preferences.setStringToSF(
        Constant.SERVER_BRANCH_ID, branchid.toString());
    await Preferences.setStringToSF(
        Constant.SERVER_SHIFT_ID, shiftid.toString());
  }

  static checkShift() async {
    // var isjoin = await CommunFun.checkIsJoinServer();
    // var isOpen;
    // if (isjoin == false) {
    var  isOpen = await Preferences.getStringValuesSF(Constant.IS_SHIFT_OPEN);
    // } else {
    //   isOpen = await shiftAPI.getshifisOpen();
    // }
    return isOpen;
  }
  ...
  static getbranchId() async {
    ...
    var branchid;
    if (isjoin == false) {
      branchid = await Preferences.getStringValuesSF(Constant.BRANCH_ID);
    } else {
      branchid = await Preferences.getStringValuesSF(Constant.SERVER_BRANCH_ID);
    }
    return branchid;
  }
