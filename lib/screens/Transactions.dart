...
class _TransactionsPageState extends State<TransactionsPage> {
  ...
  cancleTransation(reason, paymehtod) async {
    ...
    var datae = await CommunFun.getCurrentDateTime(DateTime.now());
    var res = await orderApi.updateOrderStatus(selectedOrder.app_id,
        selectedOrder.terminal_id, 3, datae, userdata.id, false);
    ...
    if (paymehtod.length > 0) {
      for (var i = 0; i < paymehtod.length; i++) {
        ...
        if (orderpayment.isCash == 1) {
          ...
          drawer.localID = uuid;
          drawer.terminalid = int.parse(terID);
  ...
  returnPayment(paymentMehtod) async {
    ...
    User userdata = await CommunFun.getuserDetails();
    var datae = await CommunFun.getCurrentDateTime(DateTime.now());
    var orderid = await orderApi.updateOrderStatus(selectedOrder.app_id,
        selectedOrder.terminal_id, 5, datae, userdata.id, true);
