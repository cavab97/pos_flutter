...
class LocalAPI {
  ...
  //     cartdetailid = await db.update("mst_cart_detail", cartdetails.toJson(),
...
*/
  ...
  //       focProduct.toJson(),
  //     );
  //     var res = await db.update("mst_cart_detail", newObj,
...
// Future<int> saveInOutDrawerData(Drawerdata drawerData) async {
  ...
  Future<List<Customer_Liquor_Inventory>> getCustomersWineInventoryTable(
      branchID) async {
    var db = DatabaseHelper.dbHelper.getDatabse();
    var query =
  ...
  Future<List<Customer_Liquor_Inventory_Log>>
      getCustomersWineInventoryLogsTable(branchID, appid) async {
    var db = DatabaseHelper.dbHelper.getDatabse();
    var query =
  ...
  Future saveSyncOrderDetails(OrderDetail orderData) async {
    ...
    if (checkisExit.length > 0) {
      orderid = await db.update("order_detail", newObj,
          where: "app_id =?  AND terminal_id =? ",
          whereArgs: [orderData.app_id, orderData.terminal_id]);
    } else {
      orderid = await db.insert("order_detail", newObj);
    }
    return orderid;
  }
