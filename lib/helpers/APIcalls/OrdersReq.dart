...
class OrdersReq {
  ...
  static placeOrder(request) async {
    ...
    try {
      ...
      List<dynamic> orderdetail = jsonDecode(data["order_details"]);
      List<dynamic> orderModifire = jsonDecode(data["order_modifire"]);
      List<dynamic> orderPay = jsonDecode(data["order_payment"]);
      List<dynamic> orderAttrs = jsonDecode(data["order_attributes"]);
      List<OrderDetail> orderDetails = orderdetail.length > 0
          ? orderdetail.map((c) => OrderDetail.fromJson(c)).toList()
          : [];
      List<OrderModifire> orderModifires = orderModifire.length > 0
          ? orderModifire.map((c) => OrderModifire.fromJson(c)).toList()
          : [];
      List<OrderAttributes> orderAttributes = orderAttrs.length > 0
          ? orderAttrs.map((c) => OrderAttributes.fromJson(c)).toList()
          : [];
      List<OrderPayment> orderPayment = orderPay.length > 0
          ? orderPay.map((c) => OrderPayment.fromJson(c)).toList()
          : [];
      Orders orderdata = Orders.fromJson(data["order"]);
      VoucherHistory history = VoucherHistory.fromJson(data["order_history"]);
      ShiftInvoice shiftInvoice = ShiftInvoice.fromJson(data["shift_invoice"]);
