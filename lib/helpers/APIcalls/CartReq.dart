...
class CartReq {
  ...
  static deletecartItem(request) async {
    ...
    try {
      ...
      MSTCartdetails details = MSTCartdetails.fromJson(data["cart_item"]);
      MST_Cart mainCart = MST_Cart.fromJson(data["main_cart"]);
      await cartlist.deleteCartItem(details, cartID, mainCart, isLast);
  ...
  static getcartLists(request) async {
    Cartlist cartlist = new Cartlist();
    try {
      String content = await utf8.decoder.bind(request).join();
      var data = await jsonDecode(content);
      var branchid = data["branch_id"];
      var res = await cartlist.getCartsList(branchid);
      request.response
        ..statusCode = HttpStatus.ok
        ..headers.contentType =
            new ContentType("json", "plain", charset: "utf-8")
