...
class ProductsList {
  ...
  Future<List<Attribute_Data>> getProductAttributes(productid) async {
    ...
    if (productid != null) {
      var qry = "SELECT product.product_id, category_attribute.name as attr_name,attributes.ca_id, group_concat(attributes.is_default) as is_default," +
  ...
  Future<dynamic> getProductDetails(
    ...
    if (isjoin == true) {
      ...
      List<dynamic> modifire = data["Modifire"];
      List<dynamic> setMealsProduct = data["SetMealsProduct"];
      List<dynamic> branchTax = data["BranchTax"];
      List<dynamic> printerList = data["PrinterList"];
      List<dynamic> cartTotals = data["CartTotals"];
      List<dynamic> cartItems = data["CartItems"];
      List<dynamic> cartSubItems = data["CartSubItems"];
      List<Attribute_Data> attData = attributes.length > 0
          ? attributes.map((c) => Attribute_Data.fromJson(c)).toList()
          : [];
      List<ModifireData> modeData = modifire.length > 0
          ? modifire.map((c) => ModifireData.fromJson(c)).toList()
          : [];
      List<SetMealProduct> setmealData = setMealsProduct.length > 0
          ? setMealsProduct.map((c) => SetMealProduct.fromJson(c)).toList()
          : [];
      List<BranchTax> branchdata = branchTax.length > 0
          ? branchTax.map((c) => BranchTax.fromJson(c)).toList()
          : [];
      List<Printer> printers = printerList.length > 0
          ? printerList.map((c) => Printer.fromJson(c)).toList()
          : [];
      List<MST_Cart> cartData = cartTotals.length > 0
          ? cartTotals.map((c) => MST_Cart.fromJson(c)).toList()
          : [];
      List<MSTCartdetails> cartitem = cartItems.isNotEmpty
          ? cartItems.map((c) => MSTCartdetails.fromJson(c)).toList()
          : [];
      List<MSTSubCartdetails> subdetails = cartSubItems.length > 0
          ? cartSubItems.map((c) => MSTSubCartdetails.fromJson(c)).toList()
          : [];
      productDetais = {
        "Attributes": attData,
        "Modifire": modeData,
        "SetMealsProduct": setmealData,
        "BranchTax": branchdata,
        "PrinterList": printers,
        "CartTotals": cartData,
        "CartItems": cartitem,
        "CartSubItems": subdetails
      };
    } else {
      // product attributes
