...
class OrdersList {
  ...
  Future<LastAppids> getLastids(terminalid) async {
    ...
    if (isjoin == true) {
      ...
      //   result = list[0];
      // }
      LastAppids ids = new LastAppids();
      var orderappid = await getLastOrderAppid(terminalid);
      var detailappid = await getLastOrdeDetailAppid(terminalid);
      var modifireid = await getLastOrderModifireAppid(terminalid);
      var attributeid = await getLastOrderAttrAppid(terminalid);
      var paymentid = await getLastOrderPaymentAppid(terminalid);
      var voucherid = await getLastVoucherHistoryid(terminalid);
      ids.app_id = orderappid;
      ids.order_detail_id = detailappid;
      ids.order_modifier_id = modifireid;
      ids.order_attr_id = attributeid;
      ids.order_payment_id = paymentid;
      ids.voucherId = voucherid;
      result = ids;
    }
    return result;
  }
  ...
  Future<int> getLastOrderAppid(terminalid) async {
  ...
  Future<int> getLastOrdeDetailAppid(terminalid) async {
    var qey =
  ...
  Future<int> getLastOrderModifireAppid(terminalid) async {
    var qey =
  ...
  Future<int> getLastOrderAttrAppid(terminalid) async {
    var qey =
  ...
  Future<int> getLastOrderPaymentAppid(terminalid) async {
    var qey =
  ...
  static orderDetails(
    ...
    try {
      ...
      for (var i = 0; i < orderDetails.length; i++) {
        ...
        var orderdetailid = await db.insert("order_detail", newObj);
        await SyncAPICalls.logActivity(
  ...
  Future<dynamic> getOrdersDetailsData(orderid, terminalid) async {
    ...
    if (isjoin == true) {
      ...
      if (result["status"] == Constant.STATUS200) {
        ...
        List<dynamic> orderdetail = data["order_items"];
        List<dynamic> orderProduct = data["order_products"];
        List<dynamic> attributes = data["order_attributes"];
        List<dynamic> modifires = data["order_modifires"];
        List<dynamic> payments = data["order_payment"];
        List<dynamic> paymentsmethod = data["order_payment_method"];
        Orders order = Orders.fromJson(data["order"]);
        List<OrderDetail> orderitem = orderdetail.length > 0
            ? orderdetail.map((c) => OrderDetail.fromJson(c)).toList()
            : [];
        List<OrderAttributes> attributedata = attributes.length > 0
            ? attributes.map((c) => OrderAttributes.fromJson(c)).toList()
            : [];
        List<OrderModifire> modifiresdata = modifires.length > 0
            ? modifires.map((c) => OrderModifire.fromJson(c)).toList()
            : [];
        List<OrderPayment> orderpaymentdata = payments.length > 0
            ? payments.map((c) => OrderPayment.fromJson(c)).toList()
            : [];
        List<Payments> paymentMethod = paymentsmethod.length > 0
            ? paymentsmethod.map((c) => Payments.fromJson(c)).toList()
            : [];
        List<ProductDetails> productdata = orderProduct.length > 0
            ? orderProduct.map((c) => ProductDetails.fromJson(c)).toList()
            : [];
        productDetais = {
          "order_payment": orderpaymentdata,
          "order_payment_method": paymentMethod,
          "order_items": orderitem,
          "order_products": productdata,
          "order": order,
          "paymentBy": User.fromJson(data["paymentBy"]),
          "order_attributes": attributedata,
          "order_modifires": modifiresdata
        };
      }
    } else {
  ...
  Future<int> updateOrderStatus(
      orderid, terminalid, status, upDate, userid, ispayment) async {
    var result;
    var isjoin = await CommunFun.checkIsJoinServer();
    if (isjoin == true) {
      var apiurl =
